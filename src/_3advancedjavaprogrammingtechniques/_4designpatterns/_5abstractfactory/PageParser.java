...
*/
/*
 * Interface alongside a corresponding Impl class in order to hide the implementation details from callers.
 * Because we'll be parsing HTML from many different web pages, we're going to need multiple instances of PageParser
 * and each instance will be instantiated with a different URL.
 * Other arguments: e.g., the download time will be the same for every single PageParser.
 *//*
